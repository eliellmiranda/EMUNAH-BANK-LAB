      ...
      *                                                               *
      * ENTRADAS:                                                     *
      *   CLIENTIN = ELIEL.EMUNAH.SEED.CLIENTES.SEQ (80 bytes)       *
      *   CONTAIN  = ELIEL.EMUNAH.SEED.CONTAS.SEQ   (100 bytes)      *
      *                                                               *
      * SAIDAS:                                                       *
      *   CLIENTE  = ELIEL.EMUNAH.ARQ.CLIENTE.KSDS  (master clientes)*
      ...
      *---------------------------------------------------------------*
       1000-ABRIR-ARQUIVOS.
           ...
           DISPLAY 'CLIENTIN FS=' WS-FS-CLIENTIN
           IF WS-FS-CLIENTIN NOT = '00'
              DISPLAY 'ERRO FATAL AO ABRIR CLIENTIN'
              MOVE 12 TO RETURN-CODE
              GOBACK
           END-IF

           OPEN INPUT CONTAS-IN
           DISPLAY 'CONTAIN  FS=' WS-FS-CONTAIN
           IF WS-FS-CONTAIN NOT = '00'
              DISPLAY 'ERRO FATAL AO ABRIR CONTAIN'
              MOVE 12 TO RETURN-CODE
              GOBACK
           END-IF

           OPEN I-O   CLIENTE-KSDS
           DISPLAY 'CLIENTE FS=' WS-FS-CLIENTE
           IF WS-FS-CLIENTE NOT = '00'
              DISPLAY 'ERRO FATAL AO ABRIR CLIENTE KSDS'
              MOVE 12 TO RETURN-CODE
              GOBACK
           END-IF

           OPEN I-O   CONTA-KSDS
           DISPLAY 'CONTA    FS=' WS-FS-CONTA
           IF WS-FS-CONTA NOT = '00'
              DISPLAY 'ERRO FATAL AO ABRIR CONTA KSDS. '
              MOVE 12 TO RETURN-CODE
              GOBACK
           END-IF

           OPEN EXTEND AUDIT-OUT
           DISPLAY 'AUDIT    FS=' WS-FS-AUDIT.
       ...
       2100-VALIDAR-CLIENTE.
           ...
           IF CLI-STATUS OF CLIENTES-IN-REG NOT = 'A'
              ...
              AND CLI-STATUS OF CLIENTES-IN-REG NOT = 'B'
               ...
               STRING
                   'CLIENTE REJEITADO - STATUS INVALIDO - ID '
           ...
           ELSE
               ...
               EVALUATE WS-FS-CLIENTE
                   ...
                   WHEN '22'
                       ...
                       STRING
                           'CLIENTE REJEITADO - DUPLICADO - ID '
                           CLI-ID-CLIENTE OF CLIENTES-IN-REG
                           DELIMITED BY SIZE
                           INTO AUDIT-REG
                       END-STRING
                   ...
                   WHEN OTHER
                       ...
                       DISPLAY '*** ERRO GRAVE GRAVACAO CLIENTE ***'
                       DISPLAY 'ID TENTOU GRAVAR: '
                               CLI-ID-CLIENTE OF CLIENTES-IN-REG
                       DISPLAY 'FILE STATUS RETORNADO: '
                               WS-FS-CLIENTE
       ...
       3100-VALIDAR-CONTA.
           ...
           IF CNT-STATUS OF CONTAS-IN-REG NOT = 'A'
              ...
              AND CNT-STATUS OF CONTAS-IN-REG NOT = 'B'
               ...
               STRING
                   'CONTA REJEITADA - STATUS INVALIDO - AG '
           ...
           ELSE
               ...
               EVALUATE WS-FS-CONTA
                   ...
                   WHEN OTHER
                       ...
                       DISPLAY '*** ERRO GRAVE GRAVACAO CONTA ***'
                       DISPLAY 'CHAVE TENTADA: '
                               CNT-CHAVE OF CONTAS-IN-REG
                       DISPLAY 'FILE STATUS RETORNADO: '
                               WS-FS-CONTA
